# Quickstart Guide: Running Quantum Chemistry Calculations with mshqc

This guide demonstrates molecular initialization and execution of quantum chemistry workflows
using the `mshqc` Python API. SCF references (RHF/UHF/ROHF) and post-HF correlation methods
(MP2/MP3, OMP2/OMP3) are both implemented as compiled C++20 kernels exposed through the Python
interface. Correlation methods use TBLIS for tensor contractions and, optionally,
Cholesky-decomposed or density-fitted integrals to keep memory footprint manageable for larger
systems.
...
2. [Molecule & Integral Engine Initialization](#2-molecule--integral-engine-initialization)
...
4. [Electron Correlation Methods](#4-electron-correlation-methods)
...
Before importing the module, activate the `mshqc` Conda environment and pin thread affinity at
the shell level. `mshqc` links against MKL, TBLIS, and OpenMP simultaneously; without explicit
pinning, each library may spawn its own thread pool and oversubscribe the available cores, which
hurts performance and can produce inconsistent timings across runs.

```bash
conda activate mshqc

# Pin thread affinity — recommended before any mshqc execution
...
## 2. Molecule & Integral Engine Initialization

Geometry parsing, basis set loading, and integral engine construction all execute eagerly —
each call returns a fully built object, with no deferred or background compilation step.
...
All SCF methods (RHF, UHF, ROHF) use DIIS convergence acceleration. The `SCFConfig` object
controls convergence thresholds, DIIS parameters, and level shifting.
...
Correlation methods build on a converged SCF reference and use TBLIS-backed tensor
contractions over the AO/MO integrals supplied by `IntegralEngine` or `CholeskyERI`.
...
first-order orbital relaxation error. Orbital updates use a trust-region SOSCF scheme
...
Third-order perturbation theory involves $O(N^6)$ scaling tensor contractions. TBLIS's
blocked contraction kernels are what keep these calculations from becoming fully
bandwidth-bound at this scaling.
...
**Illustrative output** (cc-pVTZ, H₂O at equilibrium geometry — confirm against your own build,
these are placeholder values and not a verified run):
