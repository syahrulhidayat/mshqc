...
// ============================================================================
// Single ERI Computation for Cholesky
// ============================================================================

double IntegralEngine::compute_single_eri(int mu, int nu, int lam, int sig) {
    // 1. Dapatkan referensi ke list of shells
    const auto& shells = libint_shells_->shells; // Mengakses struct internal
    
    // Helper lambda untuk mencari shell index & offset
    auto get_shell_info = [&](int bf_idx) -> std::pair<int, int> {
        // Gunakan peta shell_to_basis_function_map yang sudah ada di BasisSet
        // untuk pencarian yang lebih cepat (O(1) atau O(log N))
        // Tapi jika tidak ada akses langsung, kita loop manual:
        int current_bf = 0;
        for (size_t s = 0; s < shells.size(); ++s) {
            int size = shells[s].size();
            if (bf_idx < current_bf + size) {
                return {static_cast<int>(s), bf_idx - current_bf};
            }
            current_bf += size;
        }
        throw std::runtime_error("Basis function index out of bounds");
    };

    auto [sh_mu, off_mu] = get_shell_info(mu);
    auto [sh_nu, off_nu] = get_shell_info(nu);
    auto [sh_lam, off_lam] = get_shell_info(lam);
    auto [sh_sig, off_sig] = get_shell_info(sig);

    // 2. Siapkan Engine ERI
    // Mencari max nprim dan l dari shell yang terlibat saja (optimasi)
    int max_nprim = std::max({shells[sh_mu].nprim(), shells[sh_nu].nprim(), 
                              shells[sh_lam].nprim(), shells[sh_sig].nprim()});
    int max_l = std::max({shells[sh_mu].contr[0].l, shells[sh_nu].contr[0].l, 
                          shells[sh_lam].contr[0].l, shells[sh_sig].contr[0].l});

    libint2::Engine engine(libint2::Operator::coulomb, max_nprim, max_l, 0);

    // 3. Hitung Shell Quartet
    engine.compute(shells[sh_mu], shells[sh_nu], shells[sh_lam], shells[sh_sig]);
    const auto* buffer = engine.results()[0];

    if (buffer == nullptr) return 0.0;

    // 4. Ambil nilai spesifik dari buffer
    int n_nu = shells[sh_nu].size();
    int n_lam = shells[sh_lam].size();
    int n_sig = shells[sh_sig].size();

    // Libint2 storage order: (mu * N_nu * N_lam * N_sig) + ...
    int flat_idx = (off_mu * n_nu * n_lam * n_sig) + 
                   (off_nu * n_lam * n_sig) + 
                   (off_lam * n_sig) + 
                   off_sig;

    return buffer[flat_idx];
}

// ============================================================================
// FUNGSI TAMBAHAN UNTUK CHOLESKY (Implementasi Fallback)
// ============================================================================

// Hitung Diagonal (pq|pq) untuk semua pasang pq
Eigen::VectorXd IntegralEngine::compute_eri_diagonal() {
    // Hitung full ERI dulu (Fallback implementasi)
    auto eri_tensor = compute_eri(); 
    
    int n = basis_.n_basis_functions();
    int n_pairs = n * (n + 1) / 2;
    Eigen::VectorXd diag(n_pairs);
    
    int idx = 0;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j <= i; ++j) {
            // Diagonal element D_pq = (pq|pq)
            // Menggunakan cast (long) untuk menghindari warning Eigen
            diag(idx) = eri_tensor((long)i, (long)j, (long)i, (long)j);
            idx++;
        }
    }
    return diag;
...
// Hitung Satu Kolom (pq|rs) dimana rs (pivot) tetap
Eigen::VectorXd IntegralEngine::compute_eri_column(int pivot_index) {
    // 1. Dekode pivot_index (indeks komposit) menjadi r dan s
    int n = basis_.n_basis_functions();
    int r = 0, s = 0;
    
    // Algoritma inverse triangular index (i >= j)
    int idx = 0;
    bool found = false;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j <= i; ++j) {
    ...
    // 2. Hitung full ERI (Fallback)
    auto eri_tensor = compute_eri();
    
    int n_pairs = n * (n + 1) / 2;
    Eigen::VectorXd col(n_pairs);
    
    idx = 0;
    for (int p = 0; p < n; ++p) {
        for (int q = 0; q <= p; ++q) {
            // Ambil elemen (pq|rs)
            col(idx) = eri_tensor((long)p, (long)q, (long)r, (long)s);
            idx++;
        }
    }
    return col;
