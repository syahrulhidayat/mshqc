...
// Integrates Cholesky, Density Fitting, In-Core, and MLIR-JIT Direct Exact
// ==============================================================================
...
void BaseSCF::init_integrals() {
    ...
    if (config_.print_level > 0) {
        std::string mode_str = config_.scf_type;
...
void BaseSCF::init_integrals_cholesky() {
    ...
    if (config_.print_level > 0) std::cout << " Done. Rank: " << L_mat_.cols() << "\n";
}
...
// Implementasi BaseSCF::compute() dipindah ke subclass untuk penanganan polimorfisme yang lebih baik
SCFResult BaseSCF::compute() { return SCFResult(); } 

// ==============================================================================
// RHF IMPLEMENTATION (With Cholesky/DF/Incore & MLIR-JIT Direct)
// ==============================================================================
...
void RHF::initial_guess() {
    ...
    if (P_sad.rows() > 0 && P_sad.norm() > 1e-6) {
        ...
        C_beta_ = C_alpha_; eps_beta_ = eps_alpha_;
...
void RHF::build_fock_matrix() {
    if (config_.eri_method == "cholesky" || config_.use_df) {
            ...
            for (int K_start = 0; K_start < n_chol; K_start += chunk_size) {
                ...
                #pragma omp parallel
                {
                    ...
                    for (int k = 0; k < k_size; ++k) {
                        ...
                        // Pengganti TBLIS: Evaluasi MO-Basis via Eigen
                        if (use_mo_alg && n_alpha_ > 0) {
                            Eigen::MatrixXd T_a = L_K * C_alpha_.leftCols(n_alpha_);
                            K_priv.noalias() += T_a * T_a.transpose();
                        } else {
        ...
        if (max_dP < 1e-11) {
            ...
            for (int i = 0; i < nbasis_; ++i) for (int j = 0; j < i; ++j) dG(j, i) = dG(i, j); 
            G_accum_ += dG; F_alpha_ = H_ + G_accum_;
        }
    } else {
        // DIRECT JIT EVALUATION
    ...
    P_old_ = P_alpha_; F_beta_ = F_alpha_;
}
...
SCFResult RHF::compute() {
    ...
    for (iter_scf_ = 1; iter_scf_ <= config_.max_iterations; iter_scf_++) {
        ...
        Eigen::MatrixXd F_eff_a = (iter_scf_ <= 1) ? (0.7 * F_alpha_ + 0.3 * H_) : diis_a.extrapolate();

        solve_fock(F_eff_a, C_alpha_, eps_alpha_); occ_numbers_alpha_ = smear_electrons(eps_alpha_, n_alpha_);
        C_beta_ = C_alpha_; eps_beta_ = eps_alpha_; occ_numbers_beta_ = occ_numbers_alpha_;
        update_densities(); 
        
        double dE = std::abs(energy_ - energy_old_); double dP = (P_alpha_ - P_tot_snapshot).norm() / nbasis_;
    ...
    std::vector<int> irreps_a(nbasis_, 0);
    if (symmetrizer_) {
    ...
    if (config_.use_df && L_mat_.rows() == 0) {
        int n_aux = L_mat_.cols(); L_mat_.resize(nbasis_ * nbasis_, n_aux);
    ...
    r.irreps_alpha = irreps_a; r.irreps_beta = irreps_a; r.n_occ_alpha = n_alpha_; r.n_occ_beta = n_beta_;
    r.L_mat = L_mat_; print_final(r); return r;
}
...
UHF::UHF(const Molecule& mol, const BasisSet& basis, std::shared_ptr<IntegralEngine> integrals, std::shared_ptr<PointGroup> pg, std::shared_ptr<PetiteList> pl, int n_alpha, int n_beta, const SCFConfig& config)
    ...
    G_J_accum_a_ = Eigen::MatrixXd::Zero(nbasis_, nbasis_); G_J_accum_b_ = Eigen::MatrixXd::Zero(nbasis_, nbasis_);
...
void UHF::build_fock_matrix() { 
    if (config_.eri_method == "cholesky" || config_.use_df) {
        ...
        if (dP_tot.cwiseAbs().maxCoeff() < 1e-11) {
            ...
            int n_chol = L_mat_.cols(); bool is_ooc = (L_mat_.rows() == 0 && n_chol > 0);
            std::unique_ptr<utils::HDF5TensorIO> io = nullptr;
            if (is_ooc) io = std::make_unique<utils::HDF5TensorIO>("df_tensor.h5", utils::HDF5TensorIO::Mode::READ_ONLY);
            int chunk_size = is_ooc ? 128 : n_chol;
            ...
            for (int K_start = 0; K_start < n_chol; K_start += chunk_size) {
                ...
                #pragma omp parallel
                {
                    ...
                    Eigen::MatrixXd Ta_buf(nbasis_, nbasis_); Eigen::MatrixXd Tb_buf(nbasis_, nbasis_);
                    #pragma omp for schedule(dynamic)
                    for (int k = 0; k < k_size; ++k) {
                        int K_global = K_start + k;
                        Eigen::Map<const Eigen::MatrixXd> L_K(is_ooc ? L_chunk.col(k).data() : L_mat_.col(K_global).data(), nbasis_, nbasis_);
                        ...
                        if (use_mo_alg) {
                            if (n_alpha_ > 0) { Eigen::MatrixXd Ta = L_K * C_alpha_.leftCols(n_alpha_); Ka_priv.noalias() += Ta * Ta.transpose(); }
                            if (n_beta_ > 0) { Eigen::MatrixXd Tb = L_K * C_beta_.leftCols(n_beta_); Kb_priv.noalias() += Tb * Tb.transpose(); }
                        } else {
                    ...
                    #pragma omp critical
                    { dJ_mat += J_priv; dKa_acc += Ka_priv; dKb_acc += Kb_priv; }
                }
            } 
        ...
        if (max_dP < 1e-11) { F_alpha_ = H_ + G_accum_a_; F_beta_  = H_ + G_accum_b_; } 
        else {
            const double* __restrict__ p_dtot = dP_tot.data(); const double* __restrict__ p_da   = dPa.data(); const double* __restrict__ p_db   = dPb.data();
            const double* __restrict__ Jv = J_val_.data(); const int* __restrict__ Ji = J_ind_.data(); const size_t* __restrict__ Jp = J_ptr_.data();
            const double* __restrict__ Kv = K_val_.data(); const int* __restrict__ Ki = K_ind_.data(); const size_t* __restrict__ Kp = K_ptr_.data();

            Eigen::MatrixXd dGa = Eigen::MatrixXd::Zero(nbasis_, nbasis_); Eigen::MatrixXd dGb = Eigen::MatrixXd::Zero(nbasis_, nbasis_);
            size_t n_rows = row_map_.size();

            #pragma omp parallel
            {
                Eigen::MatrixXd Ga_local = Eigen::MatrixXd::Zero(nbasis_, nbasis_); Eigen::MatrixXd Gb_local = Eigen::MatrixXd::Zero(nbasis_, nbasis_);
                #pragma omp for schedule(dynamic, 32)
                for (size_t r = 0; r < n_rows; ++r) {
                    int mu = row_map_[r].first; int nu = row_map_[r].second;
                    if (schwarz_basis_(mu, nu) * max_dP < 1e-12) continue;
                    size_t js = Jp[r]; size_t je = Jp[r+1]; size_t ks = Kp[r]; size_t ke = Kp[r+1];

                    double vj = 0.0, ka = 0.0, kb = 0.0;
                    #pragma omp simd reduction(+:vj)
                    for (size_t k = js; k < je; ++k) vj += Jv[k] * p_dtot[Ji[k]];
                    #pragma omp simd reduction(+:ka, kb)
                    for (size_t k = ks; k < ke; ++k) { double v = Kv[k]; int idx = Ki[k]; ka += v * p_da[idx]; kb += v * p_db[idx]; }
                    Ga_local(mu, nu) += vj - ka; Gb_local(mu, nu) += vj - kb;
                }
                #pragma omp critical
                { dGa += Ga_local; dGb += Gb_local; }
            }
            for (int i = 0; i < nbasis_; ++i) { for (int j = 0; j < i; ++j) { dGa(j, i) = dGa(i, j); dGb(j, i) = dGb(i, j); } }
            G_accum_a_ += dGa; G_accum_b_ += dGb; F_alpha_ = H_ + G_accum_a_; F_beta_  = H_ + G_accum_b_;
        }
    } else {
...
SCFResult UHF::compute() {
    ...
    for (iter_scf_ = 1; iter_scf_ <= config_.max_iterations; iter_scf_++) {
        energy_old_ = energy_; Eigen::MatrixXd P_tot_snapshot = P_alpha_ + P_beta_;
        if (iter_scf_ > 2) { double de = std::abs(energy_ - energy_old_); if (de < 1e-3) thresh = 1e-9; if (de < 1e-6) thresh = 1e-13; }
        ...
        Eigen::MatrixXd F_eff_b = (iter_scf_ <= 1) ? (0.7 * F_beta_ + 0.3 * H_) : diis_b.extrapolate();

        solve_fock(F_eff_a, C_alpha_, eps_alpha_); occ_numbers_alpha_ = smear_electrons(eps_alpha_, n_alpha_);
        solve_fock(F_eff_b, C_beta_, eps_beta_); occ_numbers_beta_ = smear_electrons(eps_beta_, n_beta_);

        update_densities(); 
        
        double dE = std::abs(energy_ - energy_old_); Eigen::MatrixXd P_tot_new = P_alpha_ + P_beta_;
    ...
    if (symmetrizer_) {
        if (config_.print_level > 0) std::cout << "\n  [Symmetry] Sorting Orbitals by Irrep...\n";
        irreps_a = symmetrizer_->assign_mo_irreps(C_alpha_); irreps_b = symmetrizer_->assign_mo_irreps(C_beta_);
    }
...
void ROHF::build_fock_matrix() { 
    ...
    if (dP_tot.cwiseAbs().maxCoeff() < 1e-11) { F_alpha_ = H_ + G_accum_a_; F_beta_  = H_ + G_accum_b_; return; }
        ...
        for (int K_start = 0; K_start < n_chol; K_start += chunk_size) {
            ...
            #pragma omp parallel
            {
                ...
                for (int k = 0; k < k_size; ++k) {
                    ...
                    if (use_mo_alg) {
                        if (n_alpha_ > 0) { Eigen::MatrixXd Ta = L_K * C_alpha_.leftCols(n_alpha_); Ka_priv.noalias() += Ta * Ta.transpose(); }
                        if (n_beta_ > 0) { Eigen::MatrixXd Tb = L_K * C_alpha_.leftCols(n_beta_); Kb_priv.noalias() += Tb * Tb.transpose(); }
                    } else {
                        Ta_buf.noalias() = L_K * dPa; Ka_priv.noalias() += Ta_buf * L_K;
                        Tb_buf.noalias() = L_K * dPb; Kb_priv.noalias() += Tb_buf * L_K;
                    }
...
SCFResult ROHF::compute() {
    ...
    r.C_alpha = C_alpha_; r.C_beta = C_beta_; r.P_alpha = P_alpha_; r.P_beta = P_beta_;
    r.F_alpha = F_alpha_; r.F_beta = F_beta_; r.orbital_energies_alpha = eps_alpha_; r.orbital_energies_beta = eps_beta_;
    r.irreps_alpha = std::vector<int>(nbasis_, 0); r.irreps_beta = std::vector<int>(nbasis_, 0);
    r.n_occ_alpha = n_alpha_; r.n_occ_beta = n_beta_; r.L_mat = L_mat_; 
