...
void CholeskySACASSCF::ensure_cholesky_vectors() {
    ...
    const Eigen::MatrixXd& L_mat = chol.get_L_mat();
    int n_chol = L_mat.cols();
    L_ao_vectors_.resize(n_chol);
    
    #pragma omp parallel for
    for (int k = 0; k < n_chol; ++k) {
        L_ao_vectors_[k] = L_mat.col(k);
    }
...
SACASResult CholeskySACASSCF::compute(const Eigen::MatrixXd& initial_orbitals) {
    ...
    for (int iter = 0; iter < config_.max_iter; ++iter) {
        ...
        for(int s=0; s<config_.n_states; ++s) {
            res.state_energies[s] = es.eigenvalues()(s) + e_core; // Pastikan e_nuc sudah tidak ditambah 2x
            ...
            rdm1_avg += config_.weights[s] * rdm1_s;
        }
...
Eigen::MatrixXd CholeskySACASSCF::compute_generalized_fock(
    ...
    // --- 1. Pre-calculate Density Scalars B[k] for Coulomb (J) ---
    // Skalar B[k] = Trace(L^k * P_tot)
    Eigen::VectorXd B = Eigen::VectorXd::Zero(n_chol);
    
    #pragma omp parallel for schedule(static)
    for(int k=0; k<n_chol; ++k) {
        double val = 0.0;
        // Trace Inactive (Closed Shell: 2.0 * L_ii)
        for(int i : inact_idx) val += 2.0 * L_mo[k](i,i);
        
        // Trace Active (Contract with Active Density)
        // Loop kecil ini tidak masalah karena N_active kecil
    ...
    // --- 2. Build J and K Matrices (MATRIX OPTIMIZED) ---
    // Menggantikan loop skalar p,q dengan operasi matriks penuh.
    
    Eigen::MatrixXd J_tot = Eigen::MatrixXd::Zero(nbasis, nbasis);
    Eigen::MatrixXd K_tot = Eigen::MatrixXd::Zero(nbasis, nbasis);

    #pragma omp parallel
    {
        // Thread-local accumulation
        Eigen::MatrixXd J_priv = Eigen::MatrixXd::Zero(nbasis, nbasis);
        Eigen::MatrixXd K_priv = Eigen::MatrixXd::Zero(nbasis, nbasis);
        
        // Pre-allocate sub-matrices untuk menghindari alokasi memori berulang
        Eigen::MatrixXd L_inact(nbasis, inact_idx.size());
        Eigen::MatrixXd L_act(nbasis, act_idx.size());

        #pragma omp for schedule(dynamic)
        for(int k=0; k<n_chol; ++k) {
            const Eigen::MatrixXd& L = L_mo[k]; // Ukuran (N x N)
            
            // --- Optimize Coulomb (J) ---
            // J += B[k] * L^k
            // Ini operasi skalar * matriks, sangat cepat di Eigen
            J_priv += B(k) * L;

            // --- Optimize Exchange (K) ---
            
            // A. Inactive Exchange: K += L_inact * L_inact^T
            // Copy kolom inactive
            for(size_t i=0; i<inact_idx.size(); ++i) 
                L_inact.col(i) = L.col(inact_idx[i]);
            
            // BLAS Rank-k Update (Sangat Cepat)
            K_priv.noalias() += L_inact * L_inact.transpose();

            // B. Active Exchange: K += 0.5 * (L_act * P_act) * L_act^T
            // Copy kolom active
            for(size_t t=0; t<act_idx.size(); ++t)
                L_act.col(t) = L.col(act_idx[t]);
            
            // Perkalian matriks berantai (Chain Multiplication)
            // 1. Temp = L_act * P_act (Ukuran N x N_act)
            // 2. Res  = Temp * L_act^T (Ukuran N x N)
            if (act_idx.size() > 0) {
                K_priv.noalias() += 0.5 * (L_act * P_avg_act) * L_act.transpose();
            }
        }
