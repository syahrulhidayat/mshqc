...
// ============================================================================
// PT3 KERNEL (FIXED: SPIN SCALING & REMOVED DOUBLE DENOMINATOR)
// ============================================================================
// ============================================================================
// PT3 KERNEL (FIXED: REMOVED REDUNDANT ACTIVE TERM)
// ============================================================================
// ============================================================================
// PT3 KERNEL (FINAL: CORE + SEMI-INTERNAL ENABLED)
// ============================================================================
double CholeskySACASPT3::compute_state_pt3_optimized(
    ...
    // Ambil RDM spesifik state untuk menangkap perbedaan densitas
    Eigen::MatrixXd rdm1;
    ...
    // ------------------------------------------------------------------------
    // 1. CORE COUPLING (State-Independent in SA-CASSCF)
    // ------------------------------------------------------------------------
    double E3_pp = 0.0;
    double E3_hh = 0.0;
    double E3_ph = 0.0;
    ...
    for (int k_idx = 0; k_idx < n_inact_; ++k_idx) {
        for (int l_idx = 0; l_idx < n_inact_; ++l_idx) {
            int k = inact[k_idx]; int l = inact[l_idx];
            for (int a_idx = 0; a_idx < n_virt_; ++a_idx) {
                for (int b_idx = 0; b_idx < n_virt_; ++b_idx) {
                    ...
                    for (int i_idx = 0; i_idx < n_inact_; ++i_idx) {
                        for (int j_idx = 0; j_idx < n_inact_; ++j_idx) {
                            int i = inact[i_idx]; int j = inact[j_idx];
    ...
    // ------------------------------------------------------------------------
    // 2. SEMI-INTERNAL CORRECTION (ENABLED & STATE DEPENDENT)
    // ------------------------------------------------------------------------
    // Suku ini bergantung pada 'rdm1' (okupansi), jadi nilainya akan BEDA tiap state.
    ...
    if (!amps.t2_semi1.empty()) {
        ...
        for (int i_idx = 0; i_idx < n_inact_; ++i_idx) {
            ...
            for (int t_idx = 0; t_idx < n_act_; ++t_idx) {
                ...
                // [CRITICAL] State-Dependency Source
                double occ_t = rdm1(t_idx, t_idx);
                if (occ_t < 1e-9) continue;
                
                // Scaling Factor (2.0 Closed, 1.0 Open)
                double spin_factor = (occ_t > 1.5) ? 2.0 : 1.0;

                for (int a_idx = 0; a_idx < n_virt_; ++a_idx) {
                    for (int b_idx = 0; b_idx < n_virt_; ++b_idx) {
                        
                        // Amplitudo PT2 Semi-Internal (sudah dihitung per state)
                        ...
                        E3_semi += spin_factor * V_iatb * t2_val;
                    }
                }
            }
        }
    ...
    // ------------------------------------------------------------------------
    // 3. ACTIVE CORRECTION (DISABLED)
    // ------------------------------------------------------------------------
    // Tetap dimatikan karena Active PT3 ≈ 0 untuk CASSCF reference 
    // dan rawan double counting jika formulanya salah.
