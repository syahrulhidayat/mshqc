...
ActiveSpace ActiveSpace::CAS_Frozen(int n_frozen_orb, int n_active_orb, 
                                    int n_total_orb, int n_total_elec) {
    // Helper function to create CAS based on Frozen Orbitals count.
    // Useful for inputs where Frozen Core is the parameter (e.g. Frozen=0).
    
    // 1. Calculate Core Electrons
    int n_core_elec = n_frozen_orb * 2;
    
    if (n_core_elec > n_total_elec) {
        throw std::runtime_error("CAS_Frozen: Frozen electrons exceed total electrons");
    }

    // 2. Calculate Active Electrons
    int n_active_elec = n_total_elec - n_core_elec;
    
    // 3. Calculate Virtual Orbitals
    int n_virtual = n_total_orb - n_frozen_orb - n_active_orb;

    if (n_virtual < 0) {
        throw std::runtime_error("CAS_Frozen: Not enough orbitals for requested active space");
    }

    // Return using main constructor (validation happens inside)
    return ActiveSpace(n_frozen_orb, n_active_orb, n_virtual, n_active_elec);
}
