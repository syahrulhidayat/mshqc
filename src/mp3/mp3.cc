...
void OMP3::build_generalized_fock() {
    ...
    if (is_restricted) {
        ...
        // Explicit OVOV Gamma (T2 x T2 only)
        ...
        // Explicit VVVV Gamma (T2 x T2 only) -> Total factor = 1.0
        ...
        // Explicit OOOO Gamma (T2 x T2 only) -> Total factor = -1.0
        ...
        // Teff (Z-vector separable part) = T2 + 2.0 * T3
        ...
        // Teff (Lambda Z-vector separable response) = T2 + 2.0 * T3
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> Teff_aa(na_*va_, na_*va_);
        #pragma omp parallel for collapse(2) schedule(static)
        for (int i = 0; i < na_; ++i) {
            for (int a = 0; a < va_; ++a) {
