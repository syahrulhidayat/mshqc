...
void OMP3::build_generalized_fock() {
    ...
    // -------------------------------------------------------------------------
    // INISIALISASI T_EFF DAN GOVOV (RING TERMS) SEBELUM TAHAP 2.1
    // -------------------------------------------------------------------------
    if (t_aa_dense) {
    ...
    // --- SUNTIKKAN RING TERMS (Govov) LANGSUNG KE T_EFF SEBELUM TAHAP 2.1 ---
    ...
    if (is_restricted) {
        ...
        for (int i = 0; i < na_; ++i) {
            for (int a = 0; a < va_; ++a) {
                for (int k = 0; k < na_; ++k) {
                    for (int c = 0; c < va_; ++c) {
                        Teff_aa(i * va_ + a, k * va_ + c) += Govov_a(i, a, k, c);
                    }
                }
            }
        }
        ...
        for (int i = 0; i < na_; ++i) {
            for (int a = 0; a < va_; ++a) {
                for (int k = 0; k < na_; ++k) {
                    for (int c = 0; c < va_; ++c) {
                        Teff_aa(i * va_ + a, k * va_ + c) += Govov_aa(i, a, k, c);
                    }
                }
                for (int k = 0; k < nb_; ++k) {
                    for (int c = 0; c < vb_; ++c) {
                        Teff_ab(i * va_ + a, k * vb_ + c) += Govov_ab(i, a, k, c);
                    }
                }
            }
        }
        ...
        for (int i = 0; i < nb_; ++i) {
            for (int a = 0; a < vb_; ++a) {
                for (int k = 0; k < nb_; ++k) {
                    for (int c = 0; c < vb_; ++c) {
                        Teff_bb(i * vb_ + a, k * vb_ + c) += Govov_bb(i, a, k, c);
                    }
                }
            }
        }
