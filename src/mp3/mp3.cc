...
void OMP3::build_generalized_fock() {
    ...
    // 2. KOREKSI TEFF_AA - Tanpa double spin-adaptation!
    #pragma omp parallel for collapse(2) schedule(static)
    for (int i = 0; i < na_; ++i) {
        for (int a = 0; a < va_; ++a) {
            for (int j = 0; j < na_; ++j) {
                for (int b = 0; b < va_; ++b) {
                    double t2_sa = T2_aa_ijab(i, j, a, b);
                    double t3_sa = L2_aa_(i, j, a, b);
                    if (is_restricted) {
                        t2_sa = 2.0 * t2_sa - T2_aa_ijab(i, j, b, a);
                        t3_sa = 2.0 * t3_sa - L2_aa_(i, j, b, a);
                    }
                    // Gamma sudah berskala benar, tambahkan secara langsung!
                    Teff_aa(i*va_+a, j*va_+b) = 2.0 * t2_sa + 2.0 * t3_sa + Gamma_ovov_aa(i, a, j, b);
                }
            }
        }
    }
    ...
    if (!is_restricted && nb_ > 0 && vb_ > 0) {
        ...
        for (int i = 0; i < na_; ++i) {
            for (int a = 0; a < va_; ++a) {
                for (int j = 0; j < nb_; ++j) {
                    for (int b = 0; b < vb_; ++b) {
                        Teff_ab(i*va_+a, j*vb_+b) = 2.0 * (*t2_ab_dense)(i, j, a, b) + 2.0 * L2_ab_(i, j, a, b) + Gamma_ovov_ab(i, a, j, b);
                    }
                }
            }
        }
        ...
        for (int i = 0; i < nb_; ++i) {
            for (int a = 0; a < vb_; ++a) {
                for (int j = 0; j < nb_; ++j) {
                    for (int b = 0; b < vb_; ++b) {
                        Teff_bb(i*vb_+a, j*vb_+b) = 2.0 * (*t2_bb_dense)(i, j, a, b) + 2.0 * L2_bb_(i, j, a, b) + Gamma_ovov_bb(i, a, j, b);
                    }
                }
            }
        }
