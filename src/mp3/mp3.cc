    ...
    if (!is_restricted && nb_ > 0 && vb_ > 0) {
        ...
        for (int i = 0; i < na_; ++i) {
            for (int a = 0; a < va_; ++a) {
                for (int j = 0; j < nb_; ++j) {
                    for (int b = 0; b < vb_; ++b) {
                        double t3_part = 0.5 * L2_ab_(i, j, a, b); 
                        Teff_ab(i*va_+a, j*vb_+b) = 2.0 * ((*t2_ab_dense)(i, j, a, b) + t3_part + 0.5 * Gamma_ovov_ab(i, a, j, b)); // UBAH KE 0.5
                    }
                }
            }
        }
        ...
        for (int i = 0; i < nb_; ++i) {
            for (int a = 0; a < vb_; ++a) {
                for (int j = 0; j < nb_; ++j) {
                    for (int b = 0; b < vb_; ++b) {
                        double t3_part = 0.5 * L2_bb_(i, j, a, b); 
