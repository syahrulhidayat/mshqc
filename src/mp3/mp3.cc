...
void OMP3::build_generalized_fock() {
    ...
    if (t_aa_dense) {
        ...
        for (int i = 0; i < na_; ++i) {
            for (int j = 0; j < na_; ++j) {
                for (int a = 0; a < va_; ++a) {
                    for (int b = 0; b < va_; ++b) {
                        T2_aa_ijab(i, j, a, b) = (*t_aa_dense)(i, a, j, b);
                    }
                }
            }
        }
    ...
    if (is_restricted) {
        ...
        tblis::mult<double>(1.0, t_Gvvvv_ab, "cdab", t_Bvv_a, "bdP", 1.0, t_Yvv_b, "acP");
        ...
        tblis::mult<double>(-2.0, t_Yoo_a, "ikP", t_Bia_a, "akP", 1.0, t_Za, "ai");
        tblis::mult<double>(-2.0, t_Yoo_b, "jlP", t_Bia_b, "blP", 1.0, t_Zb, "bj");
        ...
        Eigen::MatrixXd X_a = Teff_aa * B_ia_P_alpha_ + Teff_ab * B_ia_P_beta_;
        Eigen::MatrixXd X_b = Teff_bb * B_ia_P_beta_ + Teff_ab.transpose() * B_ia_P_alpha_;
